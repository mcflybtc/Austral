...
export function makeObserver(lat: number, lon: number) {
  // @ts-ignore
  return (Astro as any).Observer
    ? new (Astro as any).Observer(lat, lon, 0)
...
export async function getPositions(
  ...
  for (const name of bodies) {
    try {
      // @ts-ignore
      const vec = (Astro as any).GeoVector
        ? (Astro as any).GeoVector(name, time, helio)
        : null;
      let eclLon: number | undefined, eclLat: number | undefined;
      // @ts-ignore
      if (vec && (Astro as any).Ecliptic) {
        const e = (Astro as any).Ecliptic(vec);
        eclLon = Number(e.elon);
        eclLat = Number(e.elat);
      }
      // @ts-ignore
      else if ((Astro as any).EclipticLongitude) {
        eclLon = Number((Astro as any).EclipticLongitude(name, time));
      }

      const sign =
        typeof eclLon === 'number' ? signFromEclipticLon(eclLon) : undefined;
      ...
      if ((Astro as any).Equator && (Astro as any).Horizon) {
        ...
        // @ts-ignore
        alt = Number(hor.altitude);
        az = Number(hor.azimuth);
        // @ts-ignore
      ...
      // @ts-ignore
      if ((Astro as any).Illumination) {
        const il = (Astro as any).Illumination(name, time);
        magnitude = il?.mag ?? null;
      }
  ...
  for (const name of bodies) {
    try {
      ...
      out.push({
        ...
        transit,
        set,
        timezone: observer.timezone || 'UTC',
      });
    } catch {
      out.push({
        name,
...
export async function getLunarPhases(month: number, year: number) {
  ...
  try {
    ...
    while (
      mq &&
      new Date(mq.time?.date || mq.time).getUTCMonth() === month - 1
    ) {
      const label =
        mq.quarter === 0
          ? 'New'
      ...
      out.push({
        phase: label,
        localTime: mq.time?.toString?.() || String(mq.time),
      });
...
export async function getSeasons(year: number) {
  try {
    ...
    if (s?.march_equinox)
      entries.push({
        event: 'MarchEquinox',
        localTime: s.march_equinox?.toString?.() || String(s.march_equinox),
      });
    if (s?.june_solstice)
      entries.push({
        event: 'JuneSolstice',
        localTime: s.june_solstice?.toString?.() || String(s.june_solstice),
      });
    if (s?.september_equinox)
      entries.push({
        event: 'SeptemberEquinox',
        localTime:
          s.september_equinox?.toString?.() || String(s.september_equinox),
      });
    if (s?.december_solstice)
      entries.push({
        event: 'DecemberSolstice',
        localTime:
          s.december_solstice?.toString?.() || String(s.december_solstice),
      });
    return { year, entries };
  } catch {
    return { year, entries: [] };
...
export async function getElongations(
  body: 'Mercury' | 'Venus',
  startFrom: Date | string,
  count = 2
) {
  ...
  try {
    ...
    if (SearchMaxElongation) {
      ...
      while (me && c < count) {
        events.push({
          time: me.time?.toString?.() || String(me.time),
          elongationDeg: me.elongation,
...
export async function getEclipses(
  observer: { latitude: number; longitude: number; timezone?: string },
  startFrom: Date | string,
  scope: 'solar-global' | 'solar-local' | 'lunar'
) {
  const date = typeof startFrom === 'string' ? new Date(startFrom) : startFrom;
  const time = makeTime(date);
  const events: any[] = [];
...
export async function getApsides(
  body: string,
  startFrom: Date | string,
  count = 2
) {
  ...
  try {
    ...
    let ap =
      body === 'Moon'
        ? SearchLunarApsis && SearchLunarApsis(time)
        : SearchPlanetApsis && SearchPlanetApsis(body, time);
    let c = 0;
    while (ap && c < count) {
      events.push({
        kind: ap.kind,
        time: ap.time?.toString?.() || String(ap.time),
        distance: ap.dist,
      });
      ap = ap.next ? ap.next() : null;
      c++;
    }
  } catch {}
  return { body, startFrom: date.toISOString(), events };
...
export async function getTransits(
  ...
  try {
    // @ts-ignore
    const t = (Astro as any).SearchTransit
      ? (Astro as any).SearchTransit(body, time)
      : null;
...
export async function getGalacticCoords(
  when: Date | string,
  bodies: BodyName[] = ['Sun', 'Moon']
) {
  const date = typeof when === 'string' ? new Date(when) : when;
...
export async function getAspectsRange(
  ...
  if (!pair) return { start: start.toISOString(), span, events };
  ...
  return { start: start.toISOString(), span, events };
}

export async function getAspects(year: number) {
  const start = new Date(Date.UTC(year, 0, 1));
  return getAspectsRange(start, 'year');
}
